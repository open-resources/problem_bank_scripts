...
source_files = []
src_dirs = []
title = topic = author = editor = date = source = template_version = problem_type = ""
attribution = outcomes = difficulty = randomization = taxonomy = ""
...
def split_file(file_content):
    ...
    question_variables = file_content[
        file_content.find(marcos_end_src) : file_content.find(problem_body_start_src)
    ]
    question_body = [
        file_content[
            file_content.find(problem_body_start_src) : file_content.rfind(problem_body_end_src)
        ]
    ]
    question_ans = re.findall(r"ANS(\(.+?[?<!)]\));", file_content)
    question_hint = [
        file_content[file_content.find(hint_start_src) : file_content.find(hint_end_src)]
    ]
    question_solution = [
        file_content[file_content.find(marcos_end_src) : file_content.find(problem_body_start_src)]
    ]
    return {
...
""".strip(
    ...
    if len(question_solution) > 0:
        server_generate_random_var = "".join(
            f'# {solution.replace("$","").replace("**", "E").strip()}\n'
...
def get_part_type(part_type):
    return {"type": part_type, "pl-customizations": {"weight": "1", "hide-answer-panel": "true"}}
...
def image_extract(question_content):
    ...
    return {"image_name": image_name, "image_alt_text": image_alt_text, "image_line_md": image_line}
        ...
        if len(question_section) > 0:
            ...
            if not hint or hint not in section_clean:
                ...
                question_raw = [
                    ...
                    if question not in image_alt_text and question
                ]
...
def help_problem_extract_ans_units(problem_subsection):
    ...
    return {"section": section_clean, "final_ans_units": final_ans_units}
...
def help_problem_extract_ans_type(problem_subsection):
    ...
    return {"ans_type": ans_type, "problem_clean": problem_ans_type_removed}


def help_problem_extract_append(problem_subsection, final_dic):
    if len(problem_subsection) > 1:
        problem_stripped = (
            problem_subsection.replace("\\", "")
            .replace("textrm", "")
            .replace("{", "")
            .replace("}", "")
            .replace("&middot;", "$\\cdot$")
    ...
    for solution in problem_solution:
        ...
        for solution_part in split_solution:
            if len(solution_part) > 0:
                ...
                if (
                    partial_answer_src not in solution_part
                    and variables_start_src not in solution_part
                ):
...
for source_filepath in source_files:
    try:
        ...
        question_formats = extract_problem_type(file_contents, dir_info["filename"])[
            "question_type"
        ]
...
print(
    "total time:",
    round(process_time_seconds / 60, 2),
    "minutes,",
    round(process_time_seconds, 2),
    "seconds",
)
print(
    "avg time per each file:",
    round(process_time_seconds / counter, 2),
    "seconds [",
    counter,
    "] files",
)
