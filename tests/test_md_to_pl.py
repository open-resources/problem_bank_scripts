...
from problem_bank_scripts import process_question_pl, process_question_md
...
def run_prairie_learn_generator(
    paths: dict[str, pathlib.Path], question: str, devmode: bool
):
    ...
    outputPath = paths["outputDest"].joinpath(
        f"prairielearn{'-dev' if devmode else ''}/"
    )
    ...
    [
        pytest.param(
            ...
            marks=(
                [
                    pytest.mark.xfail(
                        reason="Problem specified in the `exclude_question` list"
                    )
                ]
...
def test_info_json(
    paths: dict[str, pathlib.Path], question: str, devmode: bool, validate_info_json
):
    ...
    """
    run_prairie_learn_generator(paths, question, devmode)
    output_info_json = paths["outputDest"].joinpath(
        f"prairielearn{'-dev' if devmode else ''}/{question}/info.json"
    )
    compare_info_json = paths["compareDest"].joinpath(
        f"prairielearn{'-dev' if devmode else ''}/{question}/info.json"
    )
